...
export const applicationStatuses = [
  "saved", "applied", "online_assessment", "recruiter_screen", "technical_screen",
  "technical_screen_2", "technical_screen_3", "technical_screen_4", "onsite", "offer",
  "rejected", "withdrawn", "kiv",
] as const;
...
const nullableDate = z.iso.datetime({ offset: true }).transform((value) => new Date(value)).nullable().optional();
...
export type ChangeApplicationStatusInput = z.infer<typeof changeApplicationStatusInputSchema>;
...
export interface ApplicationPage { items: Application[]; total: number; limit: number; offset: number }
export interface AuditLogInput { entityType: string; entityId: string; action: string; oldValue?: unknown; newValue?: unknown }
...
export interface ApplicationsRepository {
  ...
  updateStatusWithAudit: (id: string, status: string, audit: AuditLogInput) => Promise<Application>;
...
const receivedDateStatuses = new Set(["online_assessment", "recruiter_screen", "technical_screen", "technical_screen_2", "technical_screen_3", "technical_screen_4", "onsite", "offer", "rejected"]);
const completionDateStatuses = new Set(["online_assessment", "technical_screen", "technical_screen_2", "technical_screen_3", "technical_screen_4", "onsite"]);

export class DefaultApplicationsService implements ApplicationsService {
  ...
  async create(input: CreateApplicationInput): Promise<Application> {
    ...
    if (input.role_track.trim() === "" && (input.role_tracks?.length ?? 0) === 0) throw new DomainValidationError("application track is required");
    if (input.status != null) requireStatus(input.status);
    return this.repository.create(input);
  }

  async list(): Promise<Application[]> { return this.repository.list(); }
  async get(id: string): Promise<Application> { return this.repository.get(id); }
  async delete(id: string): Promise<void> { await this.repository.delete(id); }

  async listPage(limit: number, offset: number): Promise<ApplicationPage> {
    const safeLimit = limit < 1 ? 25 : Math.min(limit, 100);
    return this.repository.listPage(safeLimit, Math.max(offset, 0));
  }
  ...
  async update(id: string, input: UpdateApplicationInput): Promise<Application> {
    ...
    if (roleTracks.length > 0 && !roleTracks.some((track) => track.trim() !== "")) throw new DomainValidationError("application track is required");
    if (input.status != null) requireStatus(input.status);
    return this.repository.update(id, input);
  }

  async changeStatus(id: string, input: ChangeApplicationStatusInput): Promise<Application> {
    const { status, received_at: receivedAt, completed_at: completedAt } = input;
    if (receivedAt != null && completedAt != null && completedAt < receivedAt) invalidDates();
    if ((receivedAt != null && !receivedDateStatuses.has(status)) || (completedAt != null && !completionDateStatuses.has(status))) invalidDates();
    const current = await this.repository.get(id);
    if (current.status === status) {
      if (receivedAt != null || completedAt != null) await this.repository.createAudit(statusAudit(id, "status_dates_recorded", status, undefined, receivedAt, completedAt));
      return current;
    }
    validateTransition(current.status, status);
    return this.repository.updateStatusWithAudit(id, status, statusAudit(id, "status_changed", status, current.status, receivedAt, completedAt));
  }

  async listAuditLogs(id: string): Promise<AuditLog[]> { return this.repository.listAuditLogs("application", id); }
}

function requireTitle(title: string): void { if (title.trim() === "") throw new DomainValidationError("application title is required"); }
function requireStatus(status: string): asserts status is ApplicationStatus {
  if (!isApplicationStatus(status)) throw new DomainValidationError("invalid application status");
}
function invalidDates(): never { throw new DomainValidationError("status completion date cannot be before received date"); }

export function validateTransition(from: string, to: string): void {
  if (!isApplicationStatus(from)) throw new DomainValidationError(`invalid application status: ${from}`);
  if (!isApplicationStatus(to)) throw new DomainValidationError(`invalid application status: ${to}`);
  if (!transitions[from].includes(to)) throw new DomainConflictError(`invalid application status transition: ${from} -> ${to}`);
}
...
function statusAudit(id: string, action: string, status: string, oldStatus?: string, receivedAt?: Date | null, completedAt?: Date | null): AuditLogInput {
  ...
  return { entityType: "application", entityId: id, action, ...(oldStatus === undefined ? {} : { oldValue: { status: oldStatus } }), newValue };
}

function auditDate(value: Date): string { return value.toISOString().replace(/\.\d{3}Z$/, "Z"); }
